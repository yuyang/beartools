...
def test_prompt_command_group_is_registered() -> None:
...
def test_prompt_check_command_filters_by_name() -> None:
    result = runner.invoke(app, ["prompt", "check", "--name", "bill_transaction_analysis"])
...
def test_prompt_eval_requires_tier(tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["prompt", "eval", str(yaml_path)])
...
def test_prompt_eval_missing_yaml_returns_error() -> None:
    result = runner.invoke(app, ["prompt", "eval", "/tmp/not-exists-prompt-eval.yaml", "--tier", "small"])
...
""",
    ...
    result = runner.invoke(app, ["prompt", "eval", str(yaml_path), "--tier", "small"])
