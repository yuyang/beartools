...
def create_eval_runner(tier: AgentTier) -> PromptEvalRunner:
    ...
    node = LLFactory().list_candidates(type="openai", model_size=tier)[0]
    client = asyncio.run(LLFactory().create_async_client(name=node.name, type="openai", model_size=node.tier))
